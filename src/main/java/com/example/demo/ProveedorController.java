...
public class ProveedorController {
    ...
    public String mostrarProveedor(
        ...
        double totalComprasMes = todasLasOrdenes.stream()
            .filter(o -> "Recibido".equals(o.getEstado())) // Solo sumamos las procesadas/recibidas[cite: 2]
            .mapToDouble(o -> o.getMontoTotal().doubleValue())
            .sum();

        java.time.LocalDate fechaActual = java.time.LocalDate.now();
        String[] meses = {"Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre"};
        String mesAnioActual = meses[fechaActual.getMonthValue() - 1] + " " + fechaActual.getYear();

        model.addAttribute("totalComprasMes", totalComprasMes);
        model.addAttribute("filtroPeriodoActivo", mesAnioActual);
